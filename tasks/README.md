...
What the benchmark runs. The survey tool resolves each YAML task into a flat task
JSON (documents inlined, budget explicit) and hands it to the exe — the job
via `run --task`, the gate via `sweep --gate`.
...
list. A user turn's content may be `{document: corpora/x.txt}`; the survey tool
...
The survey tool never trims (it can't tokenize); it inlines documents in full and
...
generated. Exactly one timed task must be defined; the survey tool refuses more.
...
- **Brain-check** (`brain_check.yaml`) is the health gate: one trivial
  three-turn task run once per `(model, provider)` before anything timed —
  inside the sweep spawn, on the sweep's own model load, so health costs no
  extra spawn. Every turn's expect must pass or the provider is marked
  unhealthy and the cell's sweep points and job are skipped. Exactly one gate
  task must be defined; the survey tool refuses more.

Accepted limitation: the gate validates plumbing up front; quality is not
re-checked at long context. This is a timing survey gated by a correctness
