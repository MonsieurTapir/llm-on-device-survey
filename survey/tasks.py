...
The survey tool inlines `{document: corpora/x.txt}` into the prompt so every backend
gets identical bytes and tokenizes them with its own tokenizer. We inline
in *full* and never trim: the survey tool can't tokenize, so any char-based trim would
