...
`backends/<dir>/backend.toml` is the *only* file the survey tool reads to learn how to
exec a backend: `{dir}` is substituted with the backend's absolute directory and
the survey tool appends the CLI subcommand + flags. No hardcoded exe paths — the
...
REGISTRY = MODELS_DIR / "models.yaml"  # the model registry + fetch spec
TASKS_DIR = REPO / "tasks"
RESULTS_DIR = REPO / "results"
SUBMISSIONS_DIR = REPO / "submissions"
PROJECT_URL = "https://github.com/MonsieurTapir/llm-on-device-survey"  # the submission tracker
...
class Backend:
    ...
    cmd: list[str]  # argv prefix; the survey tool appends `run --model … --out -`
    dir: Path
