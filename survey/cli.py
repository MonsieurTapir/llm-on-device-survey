"""`survey` — the CLI: argument parsing and dispatch only.

Each subcommand's logic lives in its own module (`survey.commands.*`, plus
`survey.fetch`); this file wires up argparse and hands off.

  plan       enumerate the cells (model × variant × provider) without running.
  run        probe each provider's ceilings; per cell one sweep spawn (which
             gates itself on the brain-check first) and, when healthy, the
             validation job; persist raw traces, aggregate to results.
  ...
  publish    stage a local run as a shareable submission under submissions/.
...
def main() -> None:
    # Windows pipes (ssh, redirects, CI) default to the legacy codepage, and the
    # output language is UTF-8 (→, ✓); replace rather than die where it can't be.
    for stream in (sys.stdout, sys.stderr):
        if hasattr(stream, "reconfigure"):
            stream.reconfigure(encoding="utf-8", errors="replace")
    ap = argparse.ArgumentParser(prog="survey", description=__doc__)
    sub = ap.add_subparsers(dest="cmd", required=True)

    def common(p: argparse.ArgumentParser) -> None:
        p.add_argument("--backend", required=True, help="backend key: llamacpp")
    ...
    p_fetch.add_argument("--only", help="comma-separated backends to fetch: llamacpp")
    ...
    p_pub.add_argument(
        "--published-dir",
        type=Path,
        default=config.SUBMISSIONS_DIR,
        help="where submissions live (default: submissions)",
    )
