...
(`../models/models.yaml`), spawns one backend process per `(model, variant, provider,
...
A package of the root uv project; run from anywhere in the repo:

```sh
uv sync
uv run survey --help

uv run survey fetch [model…] [--only llamacpp]   # pull artifacts into models/
...
uv run survey run   --backend llamacpp           # per provider: ceiling probe; per cell: gated sweep → job
uv run survey aggregate results/llamacpp-raw.json.gz   # re-derive results, no re-inference
uv run survey publish results/my-box --name my-box     # stage a submission
```
...
2. **Invoke** (`commands/run.py`, `spawn.py`) — per `(model, variant,
   provider)`: one sweep spawn that runs the brain-check gate on its own
   model load before measuring; if healthy, the validation job (S spawns).
   Reads `backends/*/backend.toml` for the invoke command.
3. **Sample** (`sampling.py`) — on the job spawns only, a background thread
   ...
   via the anchor.
4. **Aggregate** (`aggregate.py`, `metrics.py`) — pool to `[p50, max]`, fit the
   prefill cost function, and write the results JSON.

The fit (`metrics.prefill_fit`) reduces the full-width prefill chunks to an
intercept (per-dispatch cost) and a slope (the attention term) — the cost
function itself, rather than only the depths that were measured. Its `r2` and
`resid_max_pct` double as the prefill family's quality signal: that pass runs
once by design, so scatter around the line is the only available evidence that a
point was disturbed rather than real. `metrics.ubatch_points` reads the
subdivided chunks against what the fit predicts at the same depth, which is the
lane's dispatch-width sensitivity.

Raw traces are persisted before any derivation, and `survey aggregate` re-runs
...
## Shader caches: pinned, not disabled

A GPU backend builds its compute pipelines lazily, on first use, and the driver
then caches the compiled result on disk. That makes real first-launch cost —
seconds of it — a one-time charge per machine *and per exe binary*, so it shows
up on nobody's second run and is easy to mistake for noise or for a slow lane.

`spawn.py` points every known cache at a directory the tool owns
(`__GL_SHADER_DISK_CACHE_PATH`, `MESA_SHADER_CACHE_DIR`, `CUDA_CACHE_PATH`, all
set unconditionally — each is ignored by the families it doesn't belong to). It
redirects rather than disables, which is what makes the split work: each cell's
sweep gets an empty directory and pays a from-scratch compile into its `warmup`
span, then that cell's job spawns reuse the directory the sweep just populated
and run warm. Disabling instead would write nothing, and the job would pay the
compile a second time. Every provider's probe gets its own directory so it never
pre-warms a cell.
...
is a whole decode), and the tool hard-kills at `--backstop-ms`. A bad
...
carry both (`llamacpp` preallocates KV with a clean plateau).
...
`uv run ruff check survey` / `uv run ruff format survey`.
