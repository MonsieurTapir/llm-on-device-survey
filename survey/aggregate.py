...
artifact `survey run` writes (`<backend>-raw.json.gz`: per-spawn `events` + sample
series) and returns the validated results object. The live `run` path and the
standalone `survey aggregate` re-run go through the *same* `build`, so changing how
...
def sweep_result(cell_sweep: dict, *, shader_cache: str, shader_bytes: int | None) -> dict:
    ...
    `fit` reduces the full-width chunks to the two parameters of the prefill cost
    function, and `ubatch` reads the dispatch-width sensitivity off the subdivided
    ones (see `metrics`).

    `warmup_ms` is the sweep spawn's own warmup span, which walks every dispatch
    width the pass will use. The survey tool hands each cell's sweep an empty shader
    cache directory, so on a lane that compiles compute pipelines this span is the
    driver building this model's pipeline set from scratch — a real first-launch
    cost, otherwise invisible because the driver caches it per machine. On a lane
    that compiles nothing it is just the warmup's own compute; `shader_bytes`
    (what the compile left on disk, 0 when there was none) is what tells the two
    apart. `shader_cache` says whether the location was pinnable at all: on
    `unavailable` platforms (macOS) the span reflects unknown prior machine state
    and is not comparable to a pinned one."""
...
def _run_from_cell(cell: dict, sources: dict) -> dict:
    ...
    all_traces = sweep_traces + cell["job"]["spawns"]
...
def build(raw: dict) -> dict:
    ...
    aggregation entrypoint shared by live `run` and `survey aggregate`."""
