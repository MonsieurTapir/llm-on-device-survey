"""`survey bundle` — pack a local run into a single submission tarball.

The contributor-side counterpart of `survey publish`: same validation, same
...
`survey ingest` is the receiving end; the tarball layout is its contract:
...
def cmd_bundle(args: argparse.Namespace) -> None:
    ...
    if not results:
        raise SystemExit(
            ...
            f"(`survey run --backend llamacpp --out {src}`)"
        )
