"""`survey ingest` — validate a submission tarball and land it in submissions.

The receiving end of `survey bundle`, run by the maintainer (or the submission
    ...
    for member in tar.getmembers():
        ...
        if len(parts) != 2 or ".." in parts or parts[0].startswith("/"):
            raise SystemExit(
                ...
                "produced by `survey bundle` / run.sh?"
            )
        top, filename = parts
...
def _pinned_commit() -> str | None:
    """The llama.cpp commit the llamacpp backend pins, straight from its CMakeLists."""
    cml = BACKENDS_DIR / "llamacpp" / "CMakeLists.txt"
    if not cml.exists():
        return None
...
def cmd_ingest(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            "submission-<name>.tar.gz that run.sh / `survey bundle` produced"
        ) from err
    ...
    gpus = ", ".join(g for g in machine.get("gpus") or [] if g != machine["cpu"])
