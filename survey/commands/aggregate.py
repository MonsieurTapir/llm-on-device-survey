"""`survey aggregate` — re-derive results from persisted raw traces.

Inference is expensive; aggregation is cheap and changes often. `survey run`
...
`RAW_SCHEMA_VERSION` versions the trace artifact. It is internal to the tool — *not* a
    ...
    for raw_path in args.raw:
        ...
        if raw.get("schema_version") != RAW_SCHEMA_VERSION:
            raise SystemExit(
                ...
                f"survey tool expects {RAW_SCHEMA_VERSION!r}"
            )
