"""`survey publish` — stage a local run as a shareable submission.

A *submission* lives in `submissions/<name>/`, is committed via PR, and is
what the analysis notebook reads by default. This command takes a local results
dir (what `survey run --out` produced), copies its `<backend>-results.json` and
...
It only moves and describes bytes the survey tool already produced; it never touches
...
def _machine_lines(m: dict) -> list[str]:
    ...
    cpu = f"{m['cpu']} ({m['cpu_cores']}C/{m['cpu_threads']}T)"
