...
(`gguf` for `llamacpp`) a `repo`, a `common` glob list, and a
...
def providers(backend: Backend, model_path: Path) -> list[Lane]:
    ...
    try:
        ...
        raise SystemExit(
            f"`providers` returned {lanes!r} for {model_path}; expected [{{id, description}}, …]"
        ) from err
