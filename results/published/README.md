...
3. Optionally preview the report with the new submission included:
   ...
   # → results/published/report.html — a local preview, gitignored
   ```
...
The report is built by `bench_analysis.site` over this folder — every chart
...
```
results/published/
  report.html           # local preview only (gitignored); Pages builds its own
  <name>/
