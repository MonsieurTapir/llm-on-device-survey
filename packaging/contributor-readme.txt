...
is installed outside the llm-on-device-survey folder this kit sits in (plus
a per-user cache for Python itself). Models download into the models folder
next to this kit; every kit version reads the same cache, so running a newer
release later skips the big download.
