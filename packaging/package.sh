...
# ({dir}/build/survey-llamacpp) — the survey tool needs no packaged-layout special case.
set -euo pipefail
...
UV_VERSION="0.12.0" # bundled uv release; bump deliberately, the .sha256 asset verifies it
...
ARTIFACT="llm-on-device-survey-${TAG#v}-$TARGET"
# Inside the archive the kit sits one level down — llm-on-device-survey/<tag>-<target>/ —
# so every release extracts into the same llm-on-device-survey/ folder, next to
# the models/ cache the run scripts point every kit at. A new version lands as a
# sibling directory and reuses the ~8 GB of models already fetched.
DIST="$REPO/dist"
STAGE_ROOT="$DIST/$ARTIFACT.stage"
STAGE="$STAGE_ROOT/llm-on-device-survey/$TAG-$TARGET"
...
git -C "$REPO" archive HEAD pyproject.toml uv.lock .python-version README.md survey schema tasks models backends/llamacpp/backend.toml \
  | tar -x -C "$STAGE"

EXE_DIR="$STAGE/backends/llamacpp/build"
mkdir -p "$EXE_DIR"
EXE="$(find "$BUILD" -type f \( -name survey-llamacpp -o -name survey-llamacpp.exe \) | head -1)"
[ -n "$EXE" ] || fail "no survey-llamacpp produced under $BUILD"
cp "$EXE" "$EXE_DIR/"
...
case "$TARGET" in
windows-x64) VERSION_JSON="$("$EXE_DIR/survey-llamacpp.exe" version)" ;;
*) VERSION_JSON="$("$EXE_DIR/survey-llamacpp" version)" ;;
esac
...
windows-x64)
  ...
  (cd "$STAGE_ROOT" && powershell.exe -NoProfile -Command \
    "Compress-Archive -Force -Path 'llm-on-device-survey' -DestinationPath '$(cygpath -w "$ARCHIVE")'")
  ;;
*)
  ARCHIVE="$DIST/$ARTIFACT.tar.gz"
  tar -czf "$ARCHIVE" -C "$STAGE_ROOT" llm-on-device-survey
  ;;
esac
