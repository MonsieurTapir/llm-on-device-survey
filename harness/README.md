...
uv run bench fetch [model…] [--only llamacpp]                # pull artifacts into ../models
...
uv run bench run     --backend llamacpp --models ../models --out ../results
                     # per provider: ceiling probe; per cell: gated sweep → job
uv run bench aggregate ../results/llamacpp-raw.json.gz       # re-derive results, no re-inference
...
`spawn.py` points every known cache at a directory the harness owns
