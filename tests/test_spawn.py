...
The argv shapes spawn.py emits ARE the CLI contract's tool side — a drift
...
if mode == "run":
    ...
    sweep_geom = {{**geom, "memory_points": [{{
        ...
        "buffers": [{{"name": "CPU", "model_bytes": 5, "context_bytes": 2,
                     "compute_bytes": 1}}]}}]}}
    doc = {{**base, "mode": "sweep", "model": "m", "quant": "q4", "healthy": True,
...
def test_sweep_argv_shape_and_runs_unsampled(tmp_path):
    ...
    assert not Path(argv[argv.index("--gate") + 1]).exists()  # tempfile cleaned up


def test_sweep_omits_gate_when_unset(tmp_path):
    cmd = _fake_backend(tmp_path)
    result = spawn.sweep(cmd, model_path=Path("/m/model.gguf"), quant="q4", ep="cpu:0")
    assert result.events is not None and "gate" not in result.events
    assert "--gate" not in _argv(tmp_path)
