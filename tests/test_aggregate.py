...
from survey import aggregate, metrics, schema
from survey.commands.aggregate import cmd_aggregate
...
def _gate_block(*, expect_pass: bool = True, completion: str = "blue") -> dict:
    """The gate evidence a sweep spawn carries: one turn's worth of events."""
    return {
        "task": "brain-check",
        "events": [
            {
...
def _sweep_events() -> dict:
    """A healthy gated sweep: two chunks of the instrumented pass (512 tokens in
    1 s at context 0, then 1.5 s at context 512 — the marginal cost rising) and
    one decode point (4 tokens over 3 s steady → 1 tok/s)."""
...
def _raw(cells: list[dict], probes: list[dict] | None = None) -> dict:
    return {
        ...
        # The run box's sources, as `survey run` records them. Aggregation derives
...
def test_unhealthy_cell_skips_sweep_and_job():
    """A gate miss: the sweep spawn ran (its trace carries the gate evidence)
    but measured no point, and the job never spawned."""
    unhealthy_sweep = {
        **_sweep_events(),
        "healthy": False,
        "gate": _gate_block(expect_pass=False, completion="banana"),
        "prefill_chunks": [],
        "decode_points": [],
    }
    cell = {
        ...
        "provider": "cuda:0",
        "healthy": False,
        "reason": "brain-check: expect failed (got 'banana')",
        "cold_ms": None,
        "shader_cache": "redirected",
        "shader_bytes": 0,
        "sweep": {"status": "skipped", "trace": {"events": unhealthy_sweep, "samples": []}},
        "job": {"task": "summarize-large", "status": "skipped", "spawns": []},
    }
    run = aggregate.build(_raw([cell]))["runs"][0]
    assert run["healthy"] is False
    assert run["unhealthy_reason"] == "brain-check: expect failed (got 'banana')"
