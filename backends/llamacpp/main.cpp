// survey-llamacpp — the llama.cpp backend for the on-device benchmark.
//
// Implements the CLI contract over the low-level llama.h API (not generate()):
//
//   survey-llamacpp version
//   survey-llamacpp providers --model <path.gguf>
//   survey-llamacpp run   --model <path.gguf> --quant <fp16|q8|q4|q2> --ep <lane>
//                    --task <task.json> --iters <K> --out <events.json|->
//   survey-llamacpp sweep --model <path.gguf> --quant <fp16|q8|q4|q2> --ep <lane>
//                    [--gate <task.json>] --out <events.json|->
//   survey-llamacpp probe --ep <lane> --out <events.json|->
//
...
// One process = one measurement unit; the survey tool spawns per cell.
#include "llama.h"
...
// ---------------------------------------------------------------- clocks
// Durations use a monotonic clock; one wall anchor lets the survey tool map them to wall time.
...
struct Message {
    std::string              role;
    std::string              content;             // system/user text (inlined by the survey tool)
...
class Session {
  public:
    ...
    static Session open(const std::string & model_path, const Device & device,
        ...
        // No mmap (the shipped deployment configuration): weights are read into
        // ordinary allocations, so RSS carries the whole CPU-side footprint.
    ...
    // function; see `_compile_seconds` in analysis/analysis/site.py.
...
struct Cli {
    CLI::App   app{"survey-llamacpp — llama.cpp backend"};
    Arguments  args;
...
// width is the one a tight budget drops. The walk halves all the way down to one
// thread — a single core is every ladder's natural floor, and a ladder that stops
// at width/4 leaves a curve floating above the axis on any lane whose width is
// not a power of two (12 → 6 → 3, never 1). `ceiling` joins the ladder only when
// the hardware has cores the lane's default leaves unused — elsewhere it *is* the
...
std::vector<int> thread_ladder(int width, int ceiling) {
    ...
    for (int w = width; w >= 1; w /= 2) add(w);
    return ladder;
}
// Room for the one-thread rung the ladder now always ends on: the narrowest
// widths are the slowest, and 20 s cut them off on slow chips.
constexpr int64_t kThreadLadderBudgetNs = 30'000'000'000;
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        std::cerr << "survey-llamacpp: " << what << '\n';
        if (what.find("DeviceLost") != std::string::npos)
            std::cerr << "survey-llamacpp: device loss usually means the OS GPU watchdog killed a "
