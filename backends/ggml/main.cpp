  ...
  public:
    ...
    static Session open(const std::string & model_path, const Device & device,
        ...
        for (const int context_length : context_lengths) {
            ...
            std::cerr << "ggml: context-init failed at n_ctx " << context_length
...
int cmd_sweep(const Arguments & args) {
    ...
    if (!args.gate.empty()) {
        ...
        if (!healthy) std::cerr << "ggml: gate failed — skipping the sweep\n";
    }
    ...
    if (healthy) {
        ...
        // budget; the first chunk always completes.
        int depth = 0;
    ...
    if (healthy) {
        ...
        for (const int ctx : kMemoryContexts) {
            try {
                ...
                std::cerr << "ggml: memory ladder stops at n_ctx " << ctx << " ("
                          << error.what() << ")\n";
                break;
            }
        }
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        std::cerr << "bench-ggml: " << what << '\n';
        if (what.find("DeviceLost") != std::string::npos)
            std::cerr << "bench-ggml: device loss usually means the OS GPU watchdog killed a "
