...
window.taskMath = (function () {
   ...
   * the sweep's measured curve — `[depth, ms]` pairs ascending from the survey tool's
  ...
  function taskRows(pack, p) {
    ...
    pack.records.forEach(function (rec) {
       ...
       * allocated, and not the survey tool's warm pass, which is mostly a prefill of
      ...
      var shared = {
        lane: rec.lane, who: rec.who, dev_class: rec.dev_class, model: rec.model,
  ...
  function accuracyRows(pack) {
    ...
    taskRows(pack, p).forEach(function (r) {
      ...
      rows.push({
        lane: r.lane, who: r.who, dev_class: r.dev_class, model: r.model,
        quant: r.quant, backend: r.backend, rank: r.rank, phase: phases[r.metric],
