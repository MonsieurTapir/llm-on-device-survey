...
Hue means one thing on the page: the device class — discrete GPU, integrated
GPU, CPU — three validated colors that hold at any lane count. Which lane a mark
belongs to is the axis label where the chart has one, and the hover identity
(`who`: the lane with its machine) everywhere.
...
`|safe`; only repo-owned assets (css/js/vega libs) do.
    ...
    uv run --group analysis python -m analysis.site   # writes the default
    uv run --group analysis python -m analysis.site --out path.html
...
REPO = PKG.parent  # …/analysis → the repo root
...
# Device classes: the band order in the grid, fastest kind of silicon first.
CLASSES = ("discrete GPU", "integrated GPU", "CPU")

# Device-class colors — hue means one thing on the page: what kind of silicon a
# lane runs on. The three hues (and their dark steps) are validated against both
# page surfaces on every pair, so they hold in any chart form at any lane count;
# which *lane* a mark belongs to is the axis label where the chart has one and
# the hover identity (`who`) everywhere. Every chart draws these through
# `_class_scale`.
CLASS_COLORS = {"discrete GPU": "#2a78d6", "integrated GPU": "#eb6834", "CPU": "#1baf7a"}
CLASS_COLORS_DARK = {"discrete GPU": "#3987e5", "integrated GPU": "#d95926", "CPU": "#199e70"}
...
def _width(family: str, batch, decode, cores) -> str:
    ...
    read as peers. And where the width undercounts the machine, the machine is
    said too (' 4t (8C)') — a bare '4t' reads as the size of the chip rather
    than the share of it the lane used. Empty for GPU lanes, where the pool only
    serves leftover ops."""
    if family != "cpu":
        return ""
    batch, decode, cores = (int(t) if t == t and t else None for t in (batch, decode, cores))
    if batch is None or decode is None:
        return ""
    width = f" {decode}t" if batch == decode else f" {decode}t/{batch}t"
    if cores and cores > max(batch, decode):
        width += f" ({cores}C)"
    return width


def _cores_str(cores, threads) -> str:
    """A machine's core count the way the submission READMEs print it — ' (8C/8T)'."""
    return f" ({int(cores)}C/{int(threads)}T)"
...
def _with_lanes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows are never silently pooled. `who` is the lane with its machine, the one
    identity row every mark answers on hover — color says only the device class,
    so the hover is where a mark names its silicon."""
    df = df.copy()
    ...
    df["lane"] = [
        f"{_lane_chip(d, c, f)} · {f}{_width(f, b, k, n)}"
        for d, c, f, b, k, n in zip(
            df.device,
            df.cpu,
            df.family,
            df.threads_batch,
    ...
    df["who"] = [f"{lane} — {machine}" for lane, machine in zip(df.lane, df.machine, strict=True)]
    return df


def _lane_order(df: pd.DataFrame) -> list[str]:
    """Every lane, grouped by device class then named — the fixed lane order the
    charts that sort on it share, stable across models and filters."""
...
def _depth_ranges(sweeps: pd.DataFrame) -> dict[tuple[tuple, str], dict]:
    ...
    sweep points arrive in whatever order the survey tool wrote them (`[8192, 2048, 0]`
...
def _speed_rank(df: pd.DataFrame) -> dict[tuple[str, str], int]:
    ...
    `(model, lane)` — the sweep's fastest measured fill, so the ordering rests on
    the measurement, never on the validation job. Every chart with a lane axis
    sorts on it, so the reader meets the lanes in the same order wherever they
    appear. A lane whose sweep measured no generation has no position; the charts
    fall back to the end of the order."""
    rank: dict[tuple[str, str], int] = {}
    for model, g in df[df.sweep_decode_tps_max.notna()].groupby("model"):
        ordered = g.sort_values("sweep_decode_tps_max", ascending=False)
...
def _grid_rows(df: pd.DataFrame, ranges: dict[tuple[tuple, str], dict]) -> list[dict]:
    ...
    This grid is the sweep's: the two range columns carry what it measured
    (`_range_fields`), and a row's status is the *sweep's* status — the validation
    job exists to check the calculator, and the accuracy section is where it is
    read. `value` is the init column's number; the range columns' ink is `lo`/`hi`.
    ...
    columns keep the row and stay blank. Where a killed sweep still left points,
    the note names the sweep ("sweep too slow") and the interval stays real ink —
    that column's range label yields to the note, and the range stays on hover. The
    inverse gaps are also said out loud: an ok cell whose sweep left no points in a
    range column notes "no sweep data" there, and an init column with no load spans
    to read (they ride the job's spawns) notes "not measured" instead of a silent
    blank.
    ...
    for r in df.itertuples():
        ...
        well = r.status != "unhealthy" and r.sweep_status == "ok"
        # The load spans ride the job's spawns (the only place they are measured),
        # so a job that scored nothing leaves this column empty — never the row.
        init = (r.model_load_ms_p50 + r.context_init_ms_p50) / 1e3 if r.status == "ok" else None
        if r.status == "unhealthy":
            note = "unhealthy"
        elif r.sweep_status != "ok":
            note = f"sweep {str(r.sweep_status).replace('_', ' ')}"
        else:
            note = None
        ...
        for metric in ("decode", "prefill", "init"):
            ...
            if metric in RANGE_METRICS:
                ...
                elif well and span is None:
                    row["note"] = "no sweep data"
            elif well and value is None and not note:
                row["note"] = "not measured"
            rows.append(row)
    return rows


def _class_scale() -> dict:
    """The one color mapping the page has: device class → hue, in the band order.
    A fixed three-entry domain, so the mapping never shifts with the shelf — and
    never runs out of colors the way a per-lane palette did past its eighth lane."""
    return {"domain": list(CLASSES), "range": [CLASS_COLORS[c] for c in CLASSES]}
...
def _grid_tooltip(metric: str) -> list[dict]:
    """One cell's numbers on hover, under the one identity row every mark carries
    (`who` — the lane with its machine, which the axis label alone does not say).
    Model, quant and backend stay out: they are the control row's selections. What
    the marks cannot show: for a range metric, each end of the sweep *with the
    depth it was measured at* — the range is unreadable without them."""
    who = {"field": "who", "title": "lane"}
    if metric not in RANGE_METRICS:
        return [who, {"field": "value", "title": "value"}]
    return [who, {"field": "sweep_str", "title": "tok/s over depth"}]
...
def _grid_spec(rows: list[dict], controls: list[dict]) -> dict:
    ...
    axis. A lane's mark wears its device class's hue — the page's one color mapping —
    and the band it sits in says the same thing in structure.
    ...
    nothing carries its italic status note in one column (`_grid_rows`); a killed
    sweep that still left points draws its interval and the note names the sweep,
    so real ink never reads as a measurement that was not made.
    ...
    for i, (metric, title, subtitle) in enumerate(METRICS):
        ...
        color = {
            "field": "dev_class",
            "type": "nominal",
            "legend": None,  # the band headers already name the classes
            "scale": _class_scale(),
        }
        note = {
            "type": "text",
        ...
        if metric in RANGE_METRICS:
            layer = [
                # A log scale, because the lanes it holds are three orders of
                # magnitude apart: a discrete GPU reads prompts around 10,000
                # tok/s over a CPU's tens, and on a linear axis every lane but
                # one is a sliver at zero. An interval is a rule, which carries
                # no baseline to lie about, and the reading-speed anchors spread
                # out instead of piling at the origin.
                {
                    ...
                    "encoding": {
                        ...
                        "x": {
                            ...
                            "type": "quantitative",
                            "scale": {"type": "log"},
                            # few ticks: a log axis left alone prints every
                            # 1-2-5 subdivision, and neighbours collide
                            "axis": {**_value_axis("~s"), "tickCount": 3},
                        },
                        "x2": {"field": "hi"},
                        "color": color,
                        "tooltip": tooltip,
                ...
                # Multiplicative headroom is constant pixels on a log scale: room
                # for the printed ends, the same for every lane.
                {
                    "transform": [{"calculate": "datum.label_x * 1.35", "as": "headroom"}],
                ...
                # A note on a row with ink sits at the ink's edge, sharing the
                # scale; a note on a bare row sits at the plot's left edge in
                # pixels — zero is not a position a log scale has.
                {
                    "transform": [{"filter": "datum.note !== null && datum.label_x !== null"}],
                    "mark": {**note, "dx": 9},
                    "encoding": {
                        "y": y,
                        "x": {"field": "label_x", "type": "quantitative", "axis": None},
...
def _task_pack(df: pd.DataFrame, sweeps: pd.DataFrame) -> dict:
    ...
    for r in df.itertuples():
        ...
        # page cache and the context allocated. Not the warm pass — the survey tool warms
...
def _task_tooltip() -> list[dict]:
    ...
    no number at all."""
    return [
        {"field": "who", "title": "lane"},
        {"field": "value_label", "title": "value"},
        {"field": "why", "title": "note"},
    ]


def _task_spec(controls: list[dict]) -> dict:
...
# The ring every accuracy dot wears: the light surface's own hex, swapped for the
# dark surface's by the same light→dark map that re-tints the palette. It is what
# keeps two dots at the same error two dots.
SURFACE, SURFACE_DARK = "#fcfcfb", "#16181c"


def _accuracy_spec(controls: list[dict]) -> dict:
    """How far the calculator lands from the validation job: signed percent error
    on x, one row per graded phase, one dot per lane — computed in the page (the
    same `assets/tasks.js` arithmetic that fills the task grid prices the job's
    own shape and reads it against the job's measurements — `accuracyRows`).

    A distribution, not a ranking: the question this section answers is how tight
    the calculator is, and dots on a shared axis say "everything within ±5%, one
    outlier at +13" at a glance — at any lane count, at a fixed height, where a
    bar per (lane, phase) grew a page-tall column that scaled with submissions.
    Zero — prediction equals measurement — is a rule down the middle; within a
    phase's row the dots step by device class, the same order as the grid's
    bands, so same-class lanes stack near each other and a dot names its lane on
    hover. Dots wear a surface-colored ring, so two lanes at the same error stay
    two dots."""
    y = {
        "field": "phase",
        "type": "nominal",
        "title": None,
        "sort": list(ACCURACY_PHASES),
    ...
    offset = {"field": "dev_class", "type": "nominal", "scale": {"domain": list(CLASSES)}}
    color = {
        "field": "dev_class",
        "type": "nominal",
        "legend": None,  # the grid above teaches the class hues; hover names the lane
        "scale": _class_scale(),
    }
    tooltip = [
        {"field": "who", "title": "lane"},
        {"field": "err_label", "title": "error"},
    ...
    return {
        ...
        "title": {
            ...
            "anchor": "start",
            "subtitle": "one dot per lane · right of zero: predicted slower than the job measured",
            "fontSize": 12,
            "subtitleFontSize": 10,
        },
        ...
        "layer": [
            ...
            # row and the overdraw would read as a solid spine. Under the dots,
            # whose tooltips must win the pointer.
            {
                "data": {"values": [{}]},
...
# claim that nothing was compiled: the survey tool pinned an empty cache and the run
...
def _compile_seconds(row) -> float | None:
    ...
    Where the survey tool pinned the cache (`shader_cache == "redirected"`) an empty
...
def _launch_label(seconds: float | None, pinned: bool) -> str | None:
    ...
    against a cache the survey tool could not pin. Carried as text rather than formatted
...
def _launch_rows(df: pd.DataFrame) -> list[dict]:
    ...
    *pipeline compilation* is the sweep's warmup span with its own prefill netted
    ...
    only where the survey tool pinned the cache (`shader_cache == 'redirected'`); there,
    ...
    *cold first touch* is the job's `cold_start_ms`, which the survey tool measures once
...
def _launch_spec(rows: list[dict], controls: list[dict]) -> dict:
    """One-time costs per lane: two grouped bars, compilation solid and cold first
    touch half-lit, wearing the device class's hue and the lane order of the grid.
    ...
    color = {
        "field": "dev_class",
        "type": "nominal",
        "legend": None,  # the lane is the axis label; hue only says the class
        "scale": _class_scale(),
    }
    ...
    # The model is a selection; what a bar cannot say is whose machine its lane is,
    # which of the two phases it is, and how long that phase took.
    common = [
        {"field": "who", "title": "lane"},
        {"field": "phase"},
...
def _curve_spec(
    ...
    *,
    ...
    hollow marks, so a single measurement never masquerades as a trend. Color says
    the device class; `detail` keeps two same-class lanes two paths, and the hover
    is where a curve names its lane."""
    encoding = {
        ...
        "tooltip": [
            {"field": "who", "title": "lane"},
            {"field": x, "title": x_title},
            {"field": y, "title": y_title},
        ],
    }
    return {
        "$schema": VL_SCHEMA,
...
def _footprint_spec(rows: list[dict], controls: list[dict]) -> dict:
    """The allocator's reservations against context size, one line per lane, the
    device class as its hue.
...
def _thread_scalar_rows(points: list[dict]) -> list[dict]:
    """The ladder's coverage, one row per lane: which width the lane runs, and
    which widths the ladder visited. What each step *bought* stays on the charts,
    labelled on hover — a copy of it here per (lane, model) grew a table taller
    than the charts it stood under. The widths are a lane fact (every model walks
    the same machine), so the models' ladders fold into one row; there is no
    ceiling column and no 90%-of-peak column, because both were fitted asymptotes
    and this ladder stops at the widest width the lane was willing to run."""
    keyed: dict[str, dict] = {}
    for row in points:
        rec = keyed.setdefault(row["lane"], {"widths": set(), "run": set()})
        rec["widths"].add(row["threads"])
        if row["at_width"]:
            rec["run"].add(row["threads"])

    rows = []
    for lane, rec in sorted(keyed.items()):
        widths = sorted(rec["widths"])
        rows.append(
            {
                "lane": lane,
                "width": ", ".join(str(w) for w in sorted(rec["run"])) or "—",
...
def _thread_spec(
    ...
    *,
    ...
    Same frame as the cost curves (400×220, class hue, the lane on hover) and the
    ...
    color = {
        "field": "dev_class",
        "scale": _class_scale(),
        "legend": None,  # one class of silicon here — the hover names the lane
    }
    ...
    # One class of silicon shares one hue here, so the hover also carries the
    # emphasis: the pointed-at lane keeps its ink and the rest recede. Before
    # anything is hovered the selection is empty and every lane matches, so the
    # chart opens with all lanes at full ink.
    emphasis = {"condition": {"param": "lane_hover", "value": 1}, "value": 0.25}
    return {
        "$schema": VL_SCHEMA,
        "title": {
            "text": title,
...
def build(published: Path, out: Path, vega_cache: Path | None = None) -> None:
    ...
    context: dict = {
        ...
        "specs": specs,
        "palette_dark_map": {
            **{CLASS_COLORS[c]: CLASS_COLORS_DARK[c] for c in CLASSES},
    ...
    if not empty:
        df = _with_lanes(df)
        # Two healths, two jobs: `well` is what the sweep measured (the page's
        # spine — default model, lane count, the unmeasurable list), `ok` is what
        # the validation job scored (the only rows with job metrics: memory,
        # sample completions).
        well = df[(df.status != "unhealthy") & (df.sweep_status == "ok")]
        ok = df[df.status == "ok"]
        lanes = _lane_order(df)
        # Open on the model the most lanes could measure — the widest comparison.
        default_model = (
            well.groupby("model").lane.nunique().sort_values(ascending=False).index[0]
            if len(well)
        ...
        if pack["records"]:
            specs["tasks"] = _task_spec(controls)
            context["tasks"] = True
            ...
            if any(rec["measured"] for rec in pack["records"]):
                specs["accuracy"] = _accuracy_spec(controls)
                context["accuracy"] = True

        launch_rows = _launch_rows(df)
        if launch_rows:
            specs["launch"] = _launch_spec(launch_rows, controls)
            context["launch"] = True

        pre = sweeps[(sweeps.kind == "prefill") & sweeps.ttft_ms.notna()].copy()
        dec = sweeps[(sweeps.kind == "decode") & sweeps.tps_p50.gt(0)].copy()
        keep = ["lane", "who", "dev_class", "model", "quant", "backend", "single"]
        if len(pre):
        ...
        if footprint_rows:
            specs["memory-footprint"] = _footprint_spec(footprint_rows, controls)
        ...
        # The thread ladder is optional measurement: CPU lanes only. An empty
        # frame has no columns for `_with_lanes` to read, so the guard comes first.
        ...
        if not thr.empty:
            ...
            context["thread_scalars"] = _thread_scalar_rows(points)
        ...
        context["machines"] = [
            {
                "machine": r.machine,
                # The core count rides the CPU string ('Apple M1 (8C/8T)'), so a
                # lane's thread width has the machine to read against.
                "cpu": r.cpu + _cores_str(r.cpu_cores, r.cpu_threads),
                "gpu": gpus.get(r.machine) or "—",
                "ram": f"{r.ram_gb:g} GB" if r.ram_gb == r.ram_gb else "?",
            }
            for r in df.drop_duplicates("machine").itertuples()
        ]
        bad = df[(df.status == "unhealthy") | (df.sweep_status != "ok")]
        context["unusable"] = [
            {
                "lane": r.lane,
                "machine": r.machine,
                ...
                "status": "unhealthy"
                if r.status == "unhealthy"
                else f"sweep {str(r.sweep_status).replace('_', ' ')}",
            }
                ...
                [
                    {
                        ...
                        "about": "Cells whose sweep measured nothing usable: killed by the "
                        "backstop, crashed, or failed the brain-check.",
                    }
                ]
                if context["unusable"]
                else []
        ...
        context["stats"] = [
            ...
            {"v": f"{well.lane.nunique()}", "k": "lanes"},
        ]

    env = Environment(loader=PackageLoader("analysis"), autoescape=select_autoescape(default=True))
...
def main() -> None:
    ...
    ap.add_argument("--published", type=Path, default=REPO / "submissions")
